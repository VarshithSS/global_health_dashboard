    ...
    return html.Div(
        ...
        children=[
            html.Div(style={"display": "flex", "alignItems": "center", "gap": "16px"},
                     children=[
                html.Div([
                    html.Div("Global Health Dashboard", style={
...
def update_trend_entity_options(level, filters, current):
    cfg = LEVEL_CONFIG[level]
    ...
    # Remember the user's selection: keep whichever chosen entities are still
    # valid for the new options; only fall back to defaults when none survive
    # (e.g. after switching the "Compare by" level, where country names aren't
    # valid regions/income groups).
    kept = [e for e in (current or []) if e in options_pool]
    return options, (kept or default)
...
def update_age_crude_options(family, filters, current):
    crude_col, std_col = AGE_CRUDE_PAIRS[family]
    ...
    # Remember the user's selection: keep whichever chosen countries are still
    # valid, only falling back to defaults when none survive.
    kept = [c for c in (current or []) if c in pool]
    return options, (kept or default)
